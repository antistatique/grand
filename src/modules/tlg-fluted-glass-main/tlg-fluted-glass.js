...
if (!navigator.userAgent.match(/Android/i)) {
  document.addEventListener('DOMContentLoaded', function () {
    ...
    void main() {
        ...
        float amplitude = 0.09; // The amplitude of the sine wave
        rotatedUV.x += amplitude * sin(sliceProgress * 3.14159265 * 1.5) * (1.0 - 0.5 * abs(sliceProgress - 0.5));
    ...
    class Sketch {
      constructor(options) {
        ...
        this.mode = ['static', 'mouse', 'scroll'].includes(modeAttr)
          ? modeAttr
          : 'static';
        const motionAttr = this.container.getAttribute(
    ...
    document.querySelectorAll('[tlg-fluted-glass-canvas]').forEach(element => {
      if (element.querySelector('[tlg-fluted-glass-image]')) {
        ...
        console.error(
          'No [tlg-fluted-glass-image] child found within [tlg-fluted-glass-canvas] element.'
        );
      }
    });
  });
